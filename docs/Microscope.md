...
The Engagement systems on the left reference **Invocation CMT** records which abstract the business functionality from the caller. These CMT records reference 3 tiers of metadata on the *Service-side*.
  ...
  - The individual functions exposed by a Service are represented by **Method Iteration CMT** records. 
...
erDiagram
    Invocation {
        string InvocationName PK
        string InvocationPermission UK
        string InvocationCall UK
        string ServiceName
        string ServiceVersion
        string MethodName
        string MethodVersion
        string ImplementationVersion
        string InvocationAlternate
    }

    Service {
        string RecordName PK
        string ServiceName UK
        string ServiceVersion UK
        string ServiceNamespace
    }

    "Method Iteration" {
        string RecordName PK
        string ServiceRecord UK
        string MethodName UK
        string MethodVersion UK
        string DownAlternate
        string MethodAlternate
    ...
    Implementation {
        string RecordName PK
        string MethodRecord UK
        string ImplementationVersion UK
        string ImplementationClassOrFlow
    }

    Invocation }o--o{ Service : "soft lookup (name + version)"
    Invocation }o--o{ "Method Iteration" : "soft lookup (name + version)"
    Invocation }o--o{ Implementation : "soft lookup (version)"
    Service ||--o{ "Method Iteration" : "has"
    "Method Iteration" ||--o{ Implementation : "has"
```
